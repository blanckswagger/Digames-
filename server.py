...
def shuffle_and_deal():
    deck = [v for v in range(1, 5) for _ in range(4)]
    random.shuffle(deck)
    return [deck[i*4:(i+1)*4] for i in range(4)]
    ...
    try:
        async for raw in ws:
            ...
            if t == "join":
                ...
                card = msg.get("card")
                to_idx = msg.get("to_index")
                if card not in hands[from_idx] or to_idx not in range(4) or to_idx == from_idx:
                    await send_to(ws,{"type":"error","msg":"Invalide"}); continue
...
async def main():
    print(f"DIGAMES WebSocket port {PORT}")
